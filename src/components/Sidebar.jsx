...
const Sidebar = () => {
  ...
  return (
    <div className="fixed top-0 left-0 h-screen w-20 bg-gray-100 flex flex-col items-center py-6 border-r border-gray-200 z-50">
      {/* Logo */}
      <div className="w-10 h-10 bg-black rounded-xl flex items-center justify-center mb-6">
        <span className="text-white font-bold text-xs">IFA</span>
      </div>
      ...
      <div className="flex flex-col gap-2 w-full px-3 flex-1">
        {navItems.map((item) => (
          <div key={item.path} className="relative group flex justify-center">
            ...
            {/* Tooltip */}
            <div className="absolute left-full ml-3 top-1/2 -translate-y-1/2 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none z-50">
              {item.label}
            </div>
          </div>
        ))}
      ...
      <button
        ...
        title="Logout"
        className="w-12 h-12 rounded-xl flex flex-col items-center justify-center gap-1 text-gray-500 hover:bg-red-100 hover:text-red-600 transition-colors cursor-pointer"
      >
